...
;***********************************************************
;STATE:
;   0=FIX       ;Display fixed pattern, e.g. 'uPF--1'.
...
DMV:    LD      HL, STMINOR     ;In these states, STMINOR
                                ...
                                ;ignore the '-' key.  Otherwise continue at DSTEP.
        DEC     (HL)
        CALL    LOCSTNA
        JR      NZ, DSTEP
        INC     (HL)
...
EIDI:   LD      (TEMP+1), HL
        ...
        JP      TEMP+1          ;2nd M1,
                                ...
                                ;this instruction is completed.         (Editor's comment: First/current instruction of user's program)
...
SHORTP: INC     L               ;The period is 2 kHz,
        ...
        JR      Z, COUNT
                                ;L = (# of 2kHz periods) - 2*(# of 1kHz periods)
        RL      L
                                ; 0 --- NCarry (L positive)
                                ; 1 ---  Carry (L negative)
...
LOOPL:  IN      A, (KIN)
        INC     DE
        RLA
        JR      NC, LOOPL       ;Loop untili input goes high.
...
; Input:    IX points to the buffer containing the display patterns.
...
; Destroyed registers: AF, AF', BC', DE'. (see comments on SCAN)
; CALL: none.
SCAN1:
...
TEST:   DEFS    1               ;Flag, bit 0 -- set when function
